...
- **Output-side token governor** (`core/output_governor.py`, proxy + hook):
  ...
  for depth in natural language; data-driven rule catalog), 256-token cap on ack-only
  turns (never overrides caller max_tokens; `GENESIS_TINY_BUDGET=0` disables),
  diff-only convention in the diet directive, and file-echo detection (observe-only)
  on both response paths. Every decision has a telemetry counter. No invented savings
  rates: output diet is lossy by nature, input diet is lossless.
